<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if(Auth::user()->rolename === 'tandarts' || Auth::user()->rolename === 'tandarts2')
                        <x-nav-link :href="route('tandarts.index')" :active="request()->routeIs('tandarts.index')">
                            {{ __('Tandarts') }}
                        </x-nav-link>
                    @endif
                    
                    @if(Auth::user()->rolename === 'tester')
                        <x-nav-link :href="route('tester.index')" :active="request()->routeIs('tester.index')">
                            {{ __('Tester') }}
                        </x-nav-link>
                    @endif
                    
                    @if(Auth::user()->rolename === 'voorbeeld')
                        <x-nav-link :href="route('voorbeeld.index')" :active="request()->routeIs('voorbeeld.index')">
                            {{ __('Voorbeeld') }}
                        </x-nav-link>
                    @endif
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(Auth::user()->rolename === 'voorbeeld')
                <x-responsive-nav-link :href="route('voorbeeld.index')" :active="request()->routeIs('voorbeeld.index')">
                    {{ __('Voorbeeld') }}
                </x-responsive-nav-link>
            @endif
